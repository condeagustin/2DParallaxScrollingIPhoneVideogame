...
- (void)addFrameWithImage:(Image2D*)image delay:(float)delay {
	...
	if(frameTimer > [[frames objectAtIndex:currentFrame] frameDelay]) {
		...
		if(currentFrame > [frames count]-1 || currentFrame < 0) {
			...
			if(repeat && !pingPong)
				...
				currentFrame = 0;
			if(!repeat && !pingPong) {
				// If we are not repeating and no pingPong then set the current frame to 0 and stop the animation
				running = NO;
				currentFrame = 0;
			}
		}
