...
- (void)initImpl {
	...
	// Init app delegate
	appDelegate = (VideogameAppDelegate*)[[UIApplication sharedApplication] delegate];
...
- (Image2D*)getSubImageAtPoint:(CGPoint)point subImageWidth:(GLuint)subImageWidth subImageHeight:(GLuint)subImageHeight scale:(float)subImageScale {
	
	//Create a new Image instance using the texture which has been assigned to the current instance
	Image2D *subImage = [[Image2D alloc] initWithTexture:texture scale:subImageScale];
