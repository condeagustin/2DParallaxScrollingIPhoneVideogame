...
- (id)initWithImage:(Image2D*)spriteSheet spriteWidth:(GLuint)width spriteHeight:(GLuint)height spacing:(GLuint)space {
	self = [super init];
	if (self != nil) {
...
- (id)initWithImageNamed:(NSString*)spriteSheetName spriteWidth:(GLuint)width spriteHeight:(GLuint)height spacing:(GLuint)space imageScale:(float)scale {
	...
	if (self != nil) {
		...
		image = [[Image2D alloc] initWithImage:[UIImage imageNamed:spriteSheetName] scale:scale];
...
- (Image2D*)getSpriteAtX:(GLuint)x y:(GLuint)y {
...
- (Quad2*)getVerticesForSpriteAtPoint:(CGPoint)point centerOfImage:(BOOL)center{
	[image calculateVerticesAtPoint:point subImageWidth:spriteWidth subImageHeight:spriteHeight centerOfImage:center];
	return [image vertices];
}
